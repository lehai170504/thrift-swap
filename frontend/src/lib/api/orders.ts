...
export const orderApi = {
  getMyOrders: async (page = 0, size = 10): Promise<PageResponse<Order>> => {
    const { data } = await axios.get(`/orders/me?page=${page}&size=${size}`);
    return data;
  },
  
  createBuyNowOrder: async (productId: string): Promise<Order> => {
    const { data } = await axios.post(`/orders/buy-now/${productId}`);
    return data;
  },
  
  getMySales: async (page = 0, size = 10): Promise<PageResponse<Order>> => {
    const { data } = await axios.get(`/orders/sales?page=${page}&size=${size}`);
    return data;
  },
  ...
  confirmReceipt: async (orderId: string): Promise<Order> => {
    const { data } = await axios.post(`/orders/${orderId}/confirm-receipt`);
    return data;
  },

  disputeOrder: async (orderId: string, reason: string): Promise<Order> => {
    const { data } = await axios.post(`/orders/${orderId}/dispute`, { reason });
    return data;
  },

  shipOrder: async (orderId: string, trackingCode: string): Promise<Order> => {
    const { data } = await axios.post(`/orders/${orderId}/ship`, { trackingCode });
    return data;
  },

  endAuction: async (productId: string): Promise<Order> => {
    const { data } = await axios.post(`/auctions/${productId}/end`);
    return data;
  },

  getDisputedOrders: async (page = 0, size = 10, search = ''): Promise<PageResponse<Order>> => {
    const { data } = await axios.get(`/admin/orders/disputed?page=${page}&size=${size}&search=${encodeURIComponent(search)}`);
    return data;
  },

  getAllOrders: async (page = 0, size = 20, search = ''): Promise<PageResponse<Order>> => {
    const { data } = await axios.get(`/admin/orders?page=${page}&size=${size}&search=${encodeURIComponent(search)}`);
    return data;
  },

  resolveDispute: async (orderId: string, winner: 'BUYER' | 'SELLER'): Promise<Order> => {
    const { data } = await axios.post(`/admin/orders/${orderId}/resolve`, { winner });
    return data;
  }
};
