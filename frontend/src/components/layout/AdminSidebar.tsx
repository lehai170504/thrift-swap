...
import { ShieldAlert, Wallet, ShoppingBag, Users, LayoutDashboard, Package, Tags, Shield } from 'lucide-react';
      ...
      <div className="p-6">
        <div className="flex items-center gap-2 text-2xl font-black mb-10 tracking-tight">
          ...
          <div className="text-foreground leading-none font-heading">
            Thriftly
            <span className="text-muted-foreground block text-[10px] font-bold uppercase tracking-widest mt-1">Admin Portal</span>
          </div>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.path} href={item.path}>
                <div
                  className={`flex items-center gap-4 px-4 py-3.5 rounded-full transition-all duration-200 group ${isActive
